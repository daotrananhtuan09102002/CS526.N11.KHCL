    ...
    return (
        ...
        { display: props.myDisplayHistory ? 'none' : 'flex' }]}>
            ...
            <View style={styles.scrollContainer}>
                {
                    compHistory.filter(element => {
                        return element.in.includes(searchText) || element.out.includes(searchText)
                    }).map(element =>
                        <View style={styles.box}>
                            <Text style={[styles.inputText, { backgroundColor: element.inFound ? 'rgb(217,129,47)' : 'none', color: element.inFound ? 'black' : 'rgba(128,128,128,0.5)' }]}>{element.in}</Text>
                            <Text style={[styles.outputText, { backgroundColor: element.outFound ? 'rgb(217,129,47)' : 'none', color: element.outFound ? 'black' : 'white' }]}>{'=' + element.out}</Text>
                        </View>
                    )
                }
            </View>
